...
class BattleshipAttackEnv(gym.Env):
    ...
    def reset(self):
        ...
        self.agent_ships = []    # List to track agent's ships: [(start_row, start_col, size, is_horizontal)]
        self.opponent_ships = [] # List to track opponent's ships
        self.done = False
        return self.agent_shots

    def add_ship(self, row, col, size, is_horizontal, is_opponent=False):
        """Track ship placement"""
        ship_type = self.ship_types.get(size, f"Ship of size {size}")
        if is_opponent:
            self.opponent_ships.append({
                'start': (row, col),
                'size': size,
    ...
    def check_ship_sunk(self, row, col, is_opponent=False):
        """Check if a ship was sunk by the last shot"""
        ships = self.opponent_ships if not is_opponent else self.agent_ships
        shots = self.agent_shots if not is_opponent else self.opponent_shots
        
        for ship in ships:
            start_row, start_col = ship['start']
            size = ship['size']
            is_horizontal = ship['is_horizontal']
            
            # Check if shot hit this ship
            if is_horizontal:
                if (start_row == row and 
                    start_col <= col < start_col + size):
                    # Add hit to ship's hit set
                    ship['hits'].add((row, col))
                    # Check if ship is sunk
                    if len(ship['hits']) == size:
                        return True, ship['type']
            else:
                if (start_col == col and 
                    start_row <= row < start_row + size):
                    # Add hit to ship's hit set
                    ship['hits'].add((row, col))
                    # Check if ship is sunk
                    if len(ship['hits']) == size:
    ...
    def step(self, action, is_opponent=False):
        ...
        if target_board[row, col] == 1:
            ...
            ship_sunk, ship_type = self.check_ship_sunk(row, col, is_opponent)
        else:
            shots[row, col] = -1
            reward = 0
